...
In this demonstration, I will show how to add controller actions and views that allow users to update database records. We will continue to build upon the [QuizMe project](https://github.com/human-se/quiz-me-2020){:target="_blank"} from the previous demos.
...
## 1. Rendering the Form with the `edit` Action

Let's start by setting up the `edit` action along with some other boilerplate code:

Add the standard resource routes for the `McQuestionsController` actions `edit` and `update`:
...
In the `McQuestionsController` class, create an `edit` action that will retrieve (using the `find` method) the `McQuestion` record with the `id` given in the request URL (which is held in `params[:id]`), and  then, will render the `edit.html.erb` view, passing in the `McQuestion` object, like this:
...
Add an empty `update` action to be filled in later, like this:
...
Create an `edit.html.erb` file in `app/views/mc_questions`, and give it the following code:
...
Add the `respond_to` block and `if` logic using the `update` method, like this:
...
{% include pagination.html prev_page='demo-new-create.md' next_page='demo-destroy.md' %}
